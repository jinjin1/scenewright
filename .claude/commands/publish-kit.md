...
`npm run thumbnail -- <slug>` 호출 (또는 동등 코드 경로). 기본은 Remotion Episode 컴포지션의 frame 30 (오프닝 페이드업·씬 진입이 모두 끝난 1초 시점)을 PNG로 캡처. frame을 직접 지정할 때 페이드 구간(< 15)을 주면 어둡게/검게 잡혀 CLI가 경고한다.
